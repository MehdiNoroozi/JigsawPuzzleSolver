...
MEX_DEFINE(myFromDatum) (int nlhs, mxArray* plhs[],
                       int nrhs, const mxArray* prhs[]) {
	InputArguments input(nrhs, prhs, 1);
	OutputArguments output(nlhs, plhs, 2);
    caffe::Datum datum;
	ASSERT(datum.ParseFromString(input.get<string>(0)),  "Failed to parse datum.");
	//mexPrintf("\n%i, %i", nlhs, nrhs );
	//char str_temp[200];
	//sprintf(str_temp, "\n%i, %i",  nlhs, nrhs );// mxGetM(prhs[0]),  mxGetN(prhs[0]) );
	//mexErrMsgTxt(str_temp);
}
