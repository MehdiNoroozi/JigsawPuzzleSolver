...
static void write_mean(MEX_ARGS) {
 ...
 scoped_ptr<db::DB> db_(db::GetDB("lmdb"));
 scoped_ptr<caffe::db::Cursor> *cursor;
// Usage: caffe_('open_dataset', path )
static void open_dataset(MEX_ARGS) 
{
	mxCHECK( nrhs == 1 && nlhs == 0, 
			  "Usage: caffe_('open_dataset', path )");
	db_->Open(mxArrayToString(prhs[0]), db::READ);
	//scoped_ptr<db::Cursor> cursor_(db_->NewCursor());
	cursor = new  scoped_ptr<caffe::db::Cursor>(db_->NewCursor());//&(cursor_);
}

// Usage: l = caffe_('db_move_next', n )
static void db_move_next(MEX_ARGS) 
{
	mxCHECK( nrhs == 1 && nlhs == 1, 
			  "Usage: [data, labels, l] = caffe_('read_next_batch', n )");
	int n = mxGetPr(prhs[0])[0];	  
	int cntr = 0;
	while( (*cursor) ->valid() )
	{
		(*cursor)->Next();
...
// Usage: [data, labels, l] = caffe_('read_next_batch', n )
static void read_next_batch(MEX_ARGS) 
{
	mxCHECK( nrhs == 1 && nlhs == 3, 
			  "Usage: [data, labels, l] = caffe_('read_next_batch', n )");
	int n = mxGetPr(prhs[0])[0];	  
	int l = 0;
	//mexPrintf("\n nnnn =  %i", n);
	if( (*cursor) ->valid() )
	{
		...
		//mexPrintf("\n%i, %i, %i, %i", datum.width(), datum.height(), datum.channels(), 
			//							datum.label()  );
		mwSize dims[4] = {datum.width(), datum.height(), datum.channels(), n };
		plhs[0] = mxCreateNumericArray(4, dims, mxUINT8_CLASS, mxREAL);
		mwSize dims2[2] = {1, n };
		plhs[1] = mxCreateNumericArray(2, dims2, mxDOUBLE_CLASS, mxREAL);
		const std::string data = datum.data();
		//char * data_ = (char *)(&(data[0]));
		char * out_ptr = (char *)mxGetPr(plhs[0]);
		memcpy( out_ptr, (&(data[0])), datum.width()*datum.height()*datum.channels() );
		mxGetPr(plhs[1])[l] = datum.label();
		l++;
		int stride = datum.width()*datum.height()*datum.channels();
		(*cursor)->Next();
		for( int i = 1; i < n; i++ )
		{
			if( (*cursor) ->valid() )
			{
				Datum datum;
				datum.ParseFromString((*cursor)->value());
				//mexPrintf("\n%i : %i, %i, %i, %i", i , datum.width(), datum.height(), datum.channels(), 
					//					datum.label()  );
				const std::string data_ = datum.data();
				//char * data_ = (char *)(&(data[0]));
				memcpy( out_ptr + l*stride, (&(data_[0])), stride );
				mxGetPr(plhs[1])[l] = datum.label();
				(*cursor)->Next();
				l++;
			}
	...
	plhs[2] = mxCreateDoubleScalar(l);
	//mwSize dims[4] = {datum.width(), datum.height(), datum.channels() };
	//plhs[0] = mxCreateNumericArray(3, dims, mxUINT8_CLASS, mxREAL);
}


static void parse_value(MEX_ARGS) {
  mxCHECK(nrhs == 1 && nlhs == 2,
      "Usage: [data, label] = caffe_('parse_value', value )");
  Datum datum;
  datum.ParseFromString(mxArrayToString(prhs[0]));
  ...
  const std::string data = datum.data();
  char * data_ = (char *)(&(data[0]));
  mwSize dims[3] = {datum.width(), datum.height(), datum.channels() };
  plhs[0] = mxCreateNumericArray(3, dims, mxUINT8_CLASS, mxREAL);
  memcpy( mxGetPr(plhs[0]), data_, datum.width()*datum.height()*datum.channels() );
  mwSize dims2 = 1;
  plhs[1] =  mxCreateNumericArray(1, &(dims2), mxDOUBLE_CLASS, mxREAL); 
  *(mxGetPr(plhs[1])) = datum.label();
...
static handler_registry handlers[] = {
  ...
  { "read_next_batch",	  read_next_batch },
  { "db_move_next",		  db_move_next	  },
  // The end.
  { "END",                NULL            },
};
